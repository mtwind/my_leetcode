...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        if len(nums) <= 2:
            return min(nums)
        # insert front into middle
        mid, temp, minNum = int(len(nums) / 2), nums[0], nums[0]
        del nums[0]
        nums.insert(mid, temp)
        ...
        # both larger, search right
        minNum = self.findMin(nums[mid:]) if (right > temp and left > temp) else (self.findMin(nums[:mid+1]) if (right < temp and left < temp) else right)
        return minNum
